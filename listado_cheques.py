...
#     if len(sys.argv) < 5:
#         print("Para utilizar, complete los siguientes campos: python listado_cheques.py <archivo_csv> <DNI> <SALIDA> <TIPO_CHEQUE> [--estado <ESTADO>] [--fecha <FECHA_INICIO> <FECHA_FIN>]")
#         sys.exit(1)
...
def filtrar_cheques(cheques, cliente=None, estado=None):
    resultados = []
    for cheque in cheques:
        if (
            (cliente is None or cheque.get('DNI') == cliente) and
            (estado is None or cheque.get('Estado') == estado)
        ):
            resultados.append(cheque)
    return resultados
...
if __name__ == '__main__':
    if len(sys.argv) < 6:
        print("Para utilizar, complete los siguientes campos: python listado_cheques.py <archivo_csv> <DNI> <SALIDA> <TIPO_CHEQUE> [--estado <ESTADO>]")
        sys.exit(1)
    ...
    cheques = cargar_datos(archivo_csv)
    cheques_filtrados = filtrar_cheques(cheques, cliente=dni_cliente, estado=estado)
 ...
 # python listado_cheques.py cheques.csv 12345678 PANTALLA EMITIDO aprobado 1631066400 1631066400
# #PARA VER FECHA: python listado_cheques.py cheques.csv "EJEMPLO DE DNI" 55555555 PANTALLA DEPOSITADO --fecha 2021-09-12 2021-09-20
# #PARA VER ESTADO APROBADO: python listado_cheques.py cheques.csv "EJEMPLO DE DNI" 55555555 PANTALLA EMITIDO --estado aprobado          
# #PARA VER ESTADO RECHAZADO: python listado_cheques.py cheques.csv "EJEMPLO DE DNI" 55555555 PANTALLA EMITIDO --estado rechazado  

# python listado_cheques.py cheques.csv 12345678 PANTALLA EMITIDO aprobado 